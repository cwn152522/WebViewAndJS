...
//
/**
 1.提供代码中的约束、frame便捷布局
 2.提供代码中的约束、frame便捷适配
 3.提供xib中的约束快速适配
 4.提供xib中的字体快速适配
 */
 ...
 *
 *
 *  @author 陈伟南, 18-11-19 20:50:32
 *
 *  1.支持在xib中进行字体大小适配
 *  2.支持在xib对单个约束进行适配
 *  3.支持在xib中对某个父视图进行水平、竖直或组合方向进行所有约束适配
 */
...
- (UIView *(^)(void))shipeiAllSubViewsUsinglayout;//全部适配
- (UIView *(^)(void))shiPeiAllSubViews_X_W_UsingLayout;//适配水平约束
- (UIView *(^)(void))shiPeiAllSubViews_Y_H_UsingLayout;//适配竖直约束
...
//xib字体大小适配
@property(nonatomic, assign) IBInspectable BOOL adapterFont;//字体大小适配
//xib约束适配
@property(nonatomic, assign) IBInspectable BOOL adapterHScreen;//水平方向所有约束适配
@property(nonatomic, assign) IBInspectable BOOL adapterVScreen;//竖直方向所有约束适配
@property(nonatomic, assign) IBInspectable BOOL adapterHVScreen;//相当于adapterHScreen+adapterVScreen
@end
...
//单个约束适配
@property(nonatomic, assign) IBInspectable BOOL adapterScreen;//单个约束的适配
@end
