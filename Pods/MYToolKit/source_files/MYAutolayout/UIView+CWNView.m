...
#define LayoutConstraintShiPei(a)  (([[UIScreen mainScreen] respondsToSelector:@selector(nativeBounds)]?[UIScreen mainScreen].nativeBounds.size.width/[UIScreen mainScreen].nativeScale:[UIScreen mainScreen].bounds.size.width)/375.0*(a))
...
- (UIView *(^)(void))shipeiAllSubViewsUsinglayout{
    __weak typeof(self) weakSelf = self;
    UIView *(^block)(void) = ^{
...
- (UIView *(^)(void))shiPeiAllSubViews_X_W_UsingLayout{
    __weak typeof(self) weakSelf = self;
    UIView *(^block)(void) = ^{
...
- (UIView *(^)(void))shiPeiAllSubViews_Y_H_UsingLayout{
    __weak typeof(self) weakSelf = self;
    UIView *(^block)(void) = ^(){
...
- (UIView *(^)(void))shiPeiAllSubViews_XW{
    __weak typeof(self) weakSelf = self;
    UIView *(^block)(void) = ^{
...
#define AdaptedWidthValue(x) ((x)*(([[UIScreen mainScreen] respondsToSelector:@selector(nativeBounds)]?[UIScreen mainScreen].nativeBounds.size.width/[UIScreen mainScreen].nativeScale:[UIScreen mainScreen].bounds.size.width)/375.0))
#define LayoutConstraintShiPei(a)  (([[UIScreen mainScreen] respondsToSelector:@selector(nativeBounds)]?[UIScreen mainScreen].nativeBounds.size.width/[UIScreen mainScreen].nativeScale:[UIScreen mainScreen].bounds.size.width)/375.0*(a))
#pragma mark - 全部适配
- (void)setAdapterFont:(BOOL)adapterFont{
    ...
    if(adapterFont){
        if ([self isKindOfClass:[UILabel class]]) {
            ...
            label.font = [label.font fontWithSize:AdaptedWidthValue(fontSize)];
        }
        if ([self isKindOfClass:[UIButton class]]) {
            UIButton * button = (UIButton *)self;
            CGFloat fontSize = button.titleLabel.font.pointSize;
            //创建一个新的字体相同当前字体除了指定的大小
            button.titleLabel.font = [button.titleLabel.font fontWithSize:AdaptedWidthValue(fontSize)];
        }
    }
}
- (BOOL)adapterFont{
...
#pragma mark - 全部适配
- (void)setAdapterHVScreen:(BOOL)adapterHVScreen{
    objc_setAssociatedObject(self, @selector(adapterHVScreen), [NSNumber numberWithBool:adapterHVScreen], OBJC_ASSOCIATION_RETAIN);
    if(adapterHVScreen){
        [self shiPeiSelfAllUsingLayout:2];
    }
}
- (BOOL)adapterHVScreen{
    BOOL adapterHVScreen = [objc_getAssociatedObject(self, _cmd) boolValue];
    return adapterHVScreen;
}
#pragma mark - 水平适配
- (void)setAdapterHScreen:(BOOL)adapterHScreen{
    if (self.adapterHVScreen) {
        return;
    }
    objc_setAssociatedObject(self, @selector(adapterHScreen), [NSNumber numberWithBool:adapterHScreen], OBJC_ASSOCIATION_RETAIN);
    if(adapterHScreen){
        [self shiPeiSelfAllUsingLayout:0];
    }
}
- (BOOL)adapterHScreen{
    BOOL adapterHScreen = [objc_getAssociatedObject(self, _cmd) boolValue];
    return adapterHScreen;
}

#pragma mark - 竖直适配
- (void)setAdapterVScreen:(BOOL)adapterVScreen{
    if (self.adapterHVScreen) {
        return;
    }
    objc_setAssociatedObject(self, @selector(adapterVScreen), [NSNumber numberWithBool:adapterVScreen], OBJC_ASSOCIATION_RETAIN);
    if(adapterVScreen){
        [self shiPeiSelfAllUsingLayout:1];
    }
}
- (BOOL)adapterVScreen{
    BOOL adapterVScreen = [objc_getAssociatedObject(self, _cmd) boolValue];
    return adapterVScreen;
}

// 0 h 1 v 2 hv
- (void)shiPeiSelfAllUsingLayout:(int)HORV{//全部适配(自己的宽高约束、和其他控件间的约束)
    [self.constraints enumerateObjectsUsingBlock:^(__kindof NSLayoutConstraint * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (HORV == 0) {
            //水平
            if((obj.firstAttribute == NSLayoutAttributeLeft) || obj.firstAttribute == NSLayoutAttributeRight || obj.firstAttribute == NSLayoutAttributeLeading || obj.firstAttribute == NSLayoutAttributeTrailing || obj.firstAttribute == NSLayoutAttributeWidth || obj.firstAttribute == NSLayoutAttributeCenterX){
                
                if (obj.adapterScreen == NO) {
    ...
    [self shiPeiSubViewUsingLayout:self :HORV];
}
- (void)shiPeiSubViewUsingLayout:(UIView *)targetView :(int)HORV{
    //TODO: 递归适配所有视图xywh
    [[targetView subviews] enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj shiPeiSelfAllUsingLayout :HORV];
    }];
}
@end
...
- (void)setAdapterScreen:(BOOL)adapterScreen{
    ...
    if(adapterScreen){
        self.constant =  LayoutConstraintShiPei(self.constant);
    }
}
